...
class LearningAgent(Agent):
    ...
    def build_state(self):
        ...
        state = [waypoint]
        state.append(inputs['light'])
        state.append(inputs['left'])
        state.append(inputs['oncoming'])
        '''
        for key in inputs.keys():
            state.append(inputs[key])
        '''
        return tuple(state)
    ...
    def get_maxQ(self, state):
        ...
        max_action = max(actions_Q_dict.iterkeys(), key=(lambda key: actions_Q_dict[key]))
        maxQ = actions_Q_dict[max_action]
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            action = self.valid_actions[rand_num]
        else:
            rand_num = random.randint(0,100)
            if rand_num <= 100*self.epsilon:
